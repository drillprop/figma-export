...
4. **Extract the full spec in one pass** (not in waves — that's three re-do passes). One table before coding: per **TEXT** node (family/style/weight/size/lineHeight/letterSpacing/case — CTA labels often use a *different* font/weight than body); per **component/instance frame** (w×h, padding, `itemSpacing`→gap, `cornerRadius`, fills — button height/gap/icon-size live in the **frame's** `absoluteBoundingBox`+`itemSpacing`, not the text node); per **section** (padding, gaps). A gap belongs to two *direct siblings under one parent* — find those two nodes and their shared parent before quoting a number; don't flatten a nested stack into one list and credit the gap to a grandparent. Then confirm it against the bordering nodes' `absoluteBoundingBox`, since a raw `itemSpacing`/`padding*` can be a phantom (one-child frame, `SPACE_BETWEEN`, `layoutGrow`, `HUG`/fixed height) — see [`reference/mapping.md`](reference/mapping.md). Map each master in `figma-components.json` to one reusable component; variant axes → props.
